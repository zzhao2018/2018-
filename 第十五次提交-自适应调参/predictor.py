...
def predict_vm(ecs_lines, input_lines):
    ...
    data,dateList=dealdate.getDealData(middata,middateList,inputL)
      #get knn data
    dataKnn=getKnnData(data,dateList,inputL)
    #get predictSet
...
def getPredictSet(data,dataKnn,inputL,beginD,endD,dateList):
    ...
    vL=autoDealParameter(data,dataKnn,inputL,dateList,dateLen)
    loc=0
    for ele in inputL:
        #pinghua
#        vL=getParameter(ele)
        s2,s1,s0,sD=pinghua(data[ele],vL[loc],dateList)
        L=len(s1)
        p=predict(s0[L-1],s1[L-1],s2[L-1],sD[L-1],beginD,endD,vL[loc])
        #Knn
        ...
        if Sum-iSum>0.5:
            iSum+=1
        ip=int(sum(add(p,preList,w1,w2)))
        pre[ele]=ip
        loc+=1
        print ip
        print '*'*25
