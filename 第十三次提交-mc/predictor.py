...
def predict_vm(ecs_lines, input_lines):
    ...
    allData,allDateList=getDealData(data,dateList,inputL)
    #get predictSet
    pre={}
    for ele in inputL:
        #get train data
        numData=getParameterData(allData[ele],allDateList)
        length=getDateLength(inputSet[-2][0],inputSet[-1][0])
        preResult=mc_predict(numData,length)
        pre[ele]=sum(preResult)
