...
def predict_vm(ecs_lines, input_lines):
    ...
    #get predictSet
    pre,allCpu,allMem=getPredictSet(data,inputL,inputSet[-2],inputSet[-1],dateList,VmList,uList)
...
#ok
#get predictSet
def getPredictSet(data,inputL,beginD,endD,dateList,VmList,uList):
    pre={}
    w1=1
    w2=0
    allCpu=0
    ...
    for ele in inputL:
        ...
        pPre=(uList[loc]*DL*2)*qushi
        loc+=1
        #predict
        Sum=sum(p)
        ip=int(w1*Sum+w2*pPre+0.5)
        pre[ele]=ip
